...
\author{{\large Due: 11:59 pm on Friday, January 30}}
...
\textbf{Total points.} Book exercises: 16; Lab exercises 14; Overall: 30.
...
\item (2 points) Plot a histogram of $\theta$ from the \texttt{rstan} object called \texttt{pool\_output}. Describe the distribution.

\item (2 points) Visualize the posterior distributions of the $\theta_i$ with boxplots. In the plot, there should be one box and whiskers object for each $\theta_i$.

\item (2 points) Take a few minutes to look at the contents of the two files \texttt{lab-02-pool.stan} and \texttt{lab-02-nopool.stan}. How are they different?

\item (2 points) What observable quantity does the parameter $a$ represent about our prior beliefs with respect to these data? What does $b$ represent?

\item (2 points) What do we actually observe in the rat tumor data with respect to these quantities?

\item (2 points) How well do our different prior beliefs---the ones represented by the different parameter settings above---match up with the data?

\item (2 points) Why might we have observed such a difference between the two approaches when using the prior
$\text{Beta}(1,1)$?
\end{enumerate}
